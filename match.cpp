...
Match::Match(Ui::MainWindow *ui, MainWindow *mainWindow, QObject *parent)
    : QObject(parent),
    ui(ui),
    mainWindow(mainWindow)
{
    ...
    connect(ui->bt_ajouter, &QPushButton::clicked, this, &Match::on_bt_ajouter_clicked);
    connect(ui->bt_modifier, &QPushButton::clicked, this, &Match::on_bt_modifier_clicked);
    connect(ui->bt_supprimer, &QPushButton::clicked, this, &Match::on_bt_supprimer_clicked);
    connect(ui->bt_MOD, &QPushButton::clicked, this, &Match::on_bt_MOD_clicked);
    connect(ui->bt_simulation, &QPushButton::clicked, this, &Match::on_bt_simulation_clicked);
    connect(ui->bt_tri, &QPushButton::clicked, this, &Match::on_bt_tri_clicked);
    connect(ui->bt_tri_2, &QPushButton::clicked, this, &Match::on_bt_tri_2_clicked);
    connect(ui->bt_pdf, &QPushButton::clicked, this, &Match::on_bt_pdf_clicked);
    connect(ui->bt_tri_date_dec, &QPushButton::clicked, this, &Match::on_bt_tri_date_dec_clicked);
    connect(ui->bt_tri_type_dec, &QPushButton::clicked, this, &Match::on_bt_tri_type_dec_clicked);

    // Set word wrap for prediction label
    ui->lab_pre->setWordWrap(true);

    // Store the modify tab widget and remove it initially
    widget_modif = ui->tabWidget_5->widget(3);
    ui->tabWidget_5->removeTab(3);

    // Set default values for the add match form
...
Match::~Match()
{
    // No need to delete ui since it's managed by MainWindow
}
...
void Match::on_bt_ajouter_clicked()
{
    ...
    if (equipe1Nom.isEmpty() || equipe2Nom.isEmpty() || stadeNom.isEmpty() || typeMatch.isEmpty()) {
        QMessageBox::warning(mainWindow, "Erreur", "Veuillez remplir tous les champs obligatoires");
        return;
    }

    if (equipe1Nom == equipe2Nom) {
        QMessageBox::warning(mainWindow, "Erreur", "Les équipes doivent être différentes");
        return;
    }

    int idEquipe1 = getIdEquipeParNom(equipe1Nom);
    int idEquipe2 = getIdEquipeParNom(equipe2Nom);
    int idStade = getIdStadeParNom(stadeNom);

    if (idEquipe1 == -1 || idEquipe2 == -1 || idStade == -1) {
        QMessageBox::critical(mainWindow, "Erreur", "Impossible de récupérer les identifiants des entités");
        return;
    }
    ...
    if (match.ajouter()) {
        QMessageBox::information(mainWindow, "Succès", "Match ajouté avec succès");
        ui->tab_match->setModel(GesMatch::afficher());
        // Reset form
        ui->dateEdit->setDate(QDate::currentDate());
        ui->equip1_ajout->setCurrentIndex(-1);
        ...
        QMessageBox::critical(mainWindow, "Erreur", "Erreur lors de l'ajout du match");
    }
}
...
void Match::on_bt_supprimer_clicked()
{
    ...
    if (selectedIndexes.isEmpty()) {
        QMessageBox::warning(mainWindow, "Attention", "Veuillez sélectionner un match à supprimer.");
        return;
    }
    ...
    if (test) {
        QMessageBox::information(mainWindow, "OK", "Suppression effectuée.");
        ui->tab_match->setModel(Etmp.afficher()); // Refresh
    } else {
        QMessageBox::critical(mainWindow, "Erreur", "Suppression non effectuée.");
    }
}
...
void Match::on_bt_modifier_clicked()
{
    ...
    if (!index.isValid()) {
        QMessageBox::warning(mainWindow, "Attention", "Veuillez sélectionner un match.");
        return;
    }
    ...
    if (query.exec() && query.next()) {
        ...
        ui->tabWidget_5->addTab(widget_modif, "Modifier Match");
        ui->tabWidget_5->setCurrentWidget(widget_modif);
    } else {
        QMessageBox::critical(mainWindow, "Erreur SQL", "Impossible de récupérer les détails du match.\n" + query.lastError().text());
    }
}
...
void Match::on_bt_MOD_clicked()
{
    ...
    if (m.modifier()) {
        QMessageBox::information(mainWindow, "Succès", "Match modifié avec succès.");
        ui->tabWidget_5->removeTab(3);
        ui->tab_match->setModel(GesMatch::afficher());
        ui->tabWidget_5->setCurrentIndex(1); // Retour à l'onglet principal si souhaité
    } else {
        QMessageBox::critical(mainWindow, "Erreur", "Échec de la modification du match.");
    }
}
...
void Match::on_bt_pdf_clicked()
{
    QModelIndex index = ui->tab_match->currentIndex();
    if (!index.isValid()) {
        QMessageBox::warning(mainWindow, "PDF", "Veuillez sélectionner un match.");
        return;
    }
    ...
    if (!query.exec() || !query.next()) {
        QMessageBox::critical(mainWindow, "Erreur", "Impossible de récupérer les informations du match.");
        return;
    }
    ...
    QString filePath = QFileDialog::getSaveFileName(mainWindow, "Enregistrer le PDF", "", "PDF Files (*.pdf)");
    if (filePath.isEmpty())
        return;
    ...
    QMessageBox::information(mainWindow, "Succès", "PDF généré avec succès !");
}

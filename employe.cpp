...
QMap<QString, QPair<int, double>> Employe::calculerStatistiquesPoste()
{
    QMap<QString, QPair<int, double>> posteStats;

    // Step 1: Get the total number of employees
    int totalEmployees = 0;
    QSqlQuery totalQuery("SELECT COUNT(*) FROM EMPLOYE");
    if (totalQuery.exec() && totalQuery.next()) {
        totalEmployees = totalQuery.value(0).toInt();
    } else {
        qDebug() << "Error fetching total employee count:" << totalQuery.lastError().text();
        return posteStats; // Return empty map if query fails
    }

    // Step 2: Group employees by POSTE and count them
    QSqlQuery query("SELECT POSTE, COUNT(*) AS count FROM EMPLOYE GROUP BY POSTE");
    if (query.exec()) {
        while (query.next()) {
            ...
            posteStats[poste] = qMakePair(count, percentage);
        }
    } else {
        qDebug() << "Error fetching poste statistics:" << query.lastError().text();
    }
